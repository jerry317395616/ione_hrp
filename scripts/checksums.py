...
EXCLUDED_PARTS = frozenset({".git", ".pytest_cache", ".ruff_cache", "__pycache__", "node_modules"})
...
def verify_manifest(root: Path) -> list[str]:
    ...
    violations.extend(f"untracked by checksum manifest: {path}" for path in sorted(actual.keys() - expected.keys()))
    return violations
